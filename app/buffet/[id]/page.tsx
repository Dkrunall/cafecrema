...
export default function BuffetDetailPage({
  ...
  return (
    <div ref={container} className="bg-cream min-h-screen selection:bg-forest selection:text-cream relative">
      ...
      <section className="relative min-h-[100vh] flex items-center overflow-hidden pt-0">
        <div className="hero-bg-accent absolute top-0 right-0 w-1/2 h-full bg-forest/20 pointer-events-none" />
        ...
        <div className="page-wrapper w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center relative z-10">
          
          {/* Left: Content */}
          <div className="hero-content lg:col-span-6">
            <Link href="/buffet" className="text-meta mb-12 flex items-center gap-6 !text-cream/40 hover:!text-cream transition-luxury group">
              <span className="w-12 h-[1px] bg-current group-hover:w-20 transition-all duration-700" />
              THE CATALOGUE
            </Link>
            
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-medium tracking-tight leading-none mb-10">
              <span className="!text-cream">{buffet.title.split(' ')[0]}</span> <br />
              <span className="font-heading italic font-light !text-cream/40">
                {buffet.title.split(' ').slice(1).join(' ') || 'Buffet.'}
              </span>
            </h1>
            
            <p className="text-xl md:text-2xl !text-cream/60 font-light leading-relaxed mb-16 max-w-lg italic">
              {buffet.longDesc}
            </p>
          ...
          <div className="hero-image-container lg:col-span-6 relative aspect-[4/5] lg:aspect-editorial group overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)]">
            <Image
              src={buffet.image}
              alt={buffet.title}
              fill
              className="object-cover scale-105 transition-luxury group-hover:scale-110"
              priority
            />
            <div className="image-frame absolute inset-0 border-[24px] lg:border-[40px] border-cream/30 mix-blend-overlay pointer-events-none" />
            <div className="absolute bottom-12 left-12">
               <div className="bg-cream/10 backdrop-blur-md border border-cream/20 p-8 text-cream">
                  <span className="text-[10px] font-bold uppercase tracking-[0.3em] block mb-2 opacity-60">LOCATION</span>
                  <span className="text-lg font-medium tracking-tight">MAIN ATELIER</span>
               </div>
            </div>
          </div>
        ...
        {/* Scroll Hint */}
        <div className="scroll-hint absolute bottom-12 left-1/2 -translate-x-1/2 hidden lg:flex flex-col items-center gap-4 opacity-30">
          <span className="text-[9px] font-bold uppercase tracking-[0.5em] vertical-text">SCROLL</span>
          <div className="w-[1px] h-12 bg-forest" />
        </div>
      </section>
